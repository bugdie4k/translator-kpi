                                                  ...
                                                  (let ((res (loop for el1 in list1
                                                                   for el2 in list2 do
                                                                     (unless (equalp el1 el2)
                                                                       (format t "log/equal-token-essentials-list~%~S != ~S~%" el1 el2)
                                                                       (return 'not-equal)))))
                                                    (when (eq res 'not-equal) (return 'not-equal)))
                                                  (progn (format t "log/equal-token-essentials-list~%~S~%!=~%~S~%" list1 list2) 'not-equal))))
                                    (when (eq res0 'not-equal) (return 'not-equal))))
                          (progn (format t "log/equal-token-essentials-list~%~S~%!=~%~S~%" list-list1 list-list2) 'not-equal)))))
...
(deftests test-lexer ()
  (correct
   ...
   '(("PROGRAM" :PROGRAM 1 0)
     ...
     ("-" :MINUS 4 13) (1 :NUMBER-LITERAL 4 14) ("TO" :TO 4 16)
     ...
     ("ENDLOOP" :ENDLOOP 14 2) (";" :SEMICOLON 14 9)
     ("END" :END 15 0) ("." :FULL-STOP 15 3))
